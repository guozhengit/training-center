...
public final class SandboxService {
    ...
    public synchronized void cleanup(Path attemptDirectory) throws IOException {
        ...
        if (hasParentSegment(attemptDirectory)) {
    ...
    public boolean isComplete(Path attemptDirectory) throws IOException {
        ...
        try {
            ...
            if (relative.getNameCount() != 2 || hasParentSegment(relative)) {
                return false;
            }
            requireSegment(relative.getName(0).toString(), "sessionId");
            requireSegment(relative.getName(1).toString(), "attemptId");
    ...
    interface FileIdentityProvider {
        ...
        static FileIdentityProvider system() {
            return SandboxService::systemIdentity;
        }
        ...
        static FileIdentityProvider windowsFileId(
            ...
            return path -> windowsFileIdentity(
                    path,
                    processStarter,
                    timeout,
                    outputLimit);
    ...
    private static String systemIdentity(Path path) throws IOException {
        ...
        return windowsFileIdentity(
                absolute,
                ProcessStarter.system(),
                Duration.ofSeconds(5),
                4096);
    }

    private static String windowsFileIdentity(
            Path path,
            ProcessStarter processStarter,
            Duration timeout,
    ...
    private static void terminateLeafToRoot(
        ...
        leafFirst.sort(Comparator.comparingLong(SandboxService::descendantCount));
